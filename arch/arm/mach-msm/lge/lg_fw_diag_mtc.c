...
/* the following key codes are defined by LGE */
#define KEY_FOLDER_HOME 	243
#define KEY_FOLDER_MENU 	244
#define KEY_TOUCH_SEARCH 	247
#define KEY_TOUCH_BACK		248
keycode_trans_type handset_7k_keytrans_table[HANDSET_7K_KEY_TRANS_MAP_SIZE]=
{
	{KEY_BACK, '^'}, {KEY_1,'1'}, {KEY_2,'2'}, {KEY_3,'3'}, {KEY_4,'4'},
	{KEY_5,'5'}, {KEY_6,'6'}, {KEY_7,'7'}, {KEY_8,'8'}, {KEY_9,'9'},
	{KEY_0,'0'}, {KEY_BACKSPACE,'Y'}, {KEY_HOME,'!'}, {KEY_MENU,'O'}, {KEY_SEARCH, '+'},
	{KEY_Q,'q'}, {KEY_W,'w'}, {KEY_E,'e'}, {KEY_R,'r'}, {KEY_T,'t'},
	{KEY_Y,'y'}, {KEY_U,'u'}, {KEY_I,'i'}, {KEY_O,'o'}, {KEY_P,'p'},
	{KEY_LEFT,'/'}, {KEY_LEFTALT,'$' }, {KEY_A,'a'}, {KEY_S,'s'}, {KEY_D,'d'},
	{KEY_F,'f'}, {KEY_G,'g'}, {KEY_H,'h'}, {KEY_J,'j'}, {KEY_K,'k'},
	{KEY_L,'l'}, {KEY_ENTER,'='}, {KEY_UP,'L'}, {KEY_REPLY,32}, {KEY_DOWN,'R'},
	{KEY_LEFTSHIFT, '~'}, {KEY_Z,'z'}, {KEY_X,'x'}, {KEY_C,'c'}, {KEY_V,'v'},
	{KEY_B,'b'}, {KEY_N,'n'}, {KEY_M,'m'}, {KEY_DOT, 32}, {KEY_RIGHT,'V'},
	{KEY_FOLDER_MENU,'O'}, {KEY_FOLDER_HOME, '!'}, {KEY_TOUCH_BACK, '^'}, {KEY_TOUCH_SEARCH, '+'}, {KEY_SEND,'S'},
	{KEY_END,'E'}, {KEY_VOLUMEUP,'U'}, {KEY_VOLUMEDOWN,'D'}, {KEY_VIDEO_PREV,'Z'}, {KEY_CAMERA,'A'}
};
			...
			unsigned short		pkt_len )		      /* length of request packet   */
{
	...
	if( func_ptr != NULL) {
//#ifdef CONFIG_LGE_DIAG_DEBUG
#if 1 /* checking MTC progress when the debug message is disabled */
		...
		printk(KERN_ERR "%s func_ptr is NULL\n", __FUNCTION__);
	}
#if 0
	else
...
DIAG_MTC_F_rsp_type* mtc_info_req_proc(DIAG_MTC_F_req_type *pReq)
{
	...
	if(pReq->mtc_req.info.screen_id == MTC_SUB_LCD) {
...
unsigned int Handset_7k_KeycodeTrans(word input)
{
	...
	for( index=0; index < HANDSET_7K_KEY_TRANS_MAP_SIZE; index++) {
		if( handset_7k_keytrans_table[index].handset_7k_key_code == input) {
			...
			printk(KERN_INFO "[MTC] input keycode %d, mapped keycode %d\n", input, ret);
			break;
		}
	}

	if (index == HANDSET_7K_KEY_TRANS_MAP_SIZE)
		printk(KERN_ERR "%s: No key code value for %d\n", input);
